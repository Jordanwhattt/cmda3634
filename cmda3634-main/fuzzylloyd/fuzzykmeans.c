...
void fuzzykmeans(double* data, int n, double* centers, int k){
    ...
    double* sumNewCenters = (double*)malloc(2*k*sizeof(double));
    double* sumOfWeights = (double*)malloc(k*sizeof(double));
    ...
    while(delta > tol){
        ...
        MPI_Allreduce(new_centers, sumNewCenters, 2*k,
                MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        MPI_Allreduce(cluster_weights, sumOfWeights, k,
                MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

        // Normalize new cluster centers by sum of weights
        for(j=0;j<2*k;++j){
            sumNewCenters[j] /= sumOfWeights[ j/2 ];
        }
        // Find largest coordinate change
        delta = 0;
        for(j=0;j<2*k;++j){
            d = fabs(sumNewCenters[j] - centers[j]);
            if(d > delta)
                delta = d;
            centers[j] = sumNewCenters[j];
        }
    }
